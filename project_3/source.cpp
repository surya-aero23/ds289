...
// First order upwind scheme with Euler time stepping
void upwindScheme(Vector& u, Vector& u_new, real dt, real dx, int N) {
    real coefficient = dt / dx;
    ...
    for (int i = 1; i < N; i++) {
        ...
        if (u[i] >= 0) {
            u_new[i] = u[i] * ( 1 - coefficient * (u[i] - u[i - 1]) );
        } 
        
        else {
            u_new[i] = u[i] * ( 1 - coefficient * (u[i + 1] - u[i]) );
        }
    }
...
// Second order CD scheme with Euler for viscous burgers equation
void CD2_Euler_ViscousBurgers(Vector& u, Vector& u_new, real diffusion_coefficient, real dt, real dx, int N) {
    
    u_new[0] = u[0] + dt;

    for (int i = 1; i < N; i++) {
        u_new[i] = u[i] + diffusion_coefficient * dt * CD2_second({u[i - 1], u[i], u[i + 1]}, dx) - u[i] * dt * CD2_first({u[i - 1], u[i], u[i + 1]}, dx);
    }
